...
exports.MermaidThemes = exports.MermaidProcessor = void 0;
...
class MermaidProcessor {
     ...
     */
    createEnhancedTextualDiagramRepresentation(diagramCode) {
     ...
     */
    createEnhancedFlowchartText(code) {
...
`;
     ...
     */
    createEnhancedSequenceText(code) {
        const lines = code.split('\n').map(line => line.trim()).filter(line => line);
...
`;
     ...
     */
    createEnhancedClassDiagramText(code) {
        const lines = code.split('\n').map(line => line.trim()).filter(line => line);
...
`;
     ...
     */
    createEnhancedGenericDiagramText(type, code) {
...
`;
     ...
     */
    createTextualDiagramRepresentation(diagramCode) {
        const diagramType = this.detectDiagramType(diagramCode);
...
`;
     ...
     */
    createGenericDiagramText(type, code) {
        const lineCount = code.split('\n').length;
        return `
...
`;
     ...
     */
    generateMermaidSVGPlaceholder(diagramCode) {
     ...
     */
    generateDefaultDiagramSVG(diagramType) {
        return `
    ...
    async initialize() {
        ...
        try {
            logger_1.logger.info('Initializing Mermaid processor...');
            ...
            logger_1.logger.info('Mermaid processor initialized successfully');
        }
        catch (error) {
            logger_1.logger.error('Failed to initialize Mermaid processor', { error });
            throw error;
        }
    }
    /**
     ...
     */
    async processDiagram(mermaidCode, format = 'svg', options = {}) {
        if (!this.isInitialized) {
            await this.initialize();
        }
        const { width = 800, height = 600, backgroundColor = 'white', scale = 1, } = options;
        try {
            logger_1.logger.debug('Processing Mermaid diagram', { format, width, height });
            const { result: diagram, elapsed } = await utils_1.PerformanceUtils.measureAsync(async () => {
            ...
            logger_1.logger.debug(`Mermaid diagram processed in ${elapsed}ms`);
            return diagram;
        }
        catch (error) {
            logger_1.logger.error('Failed to process Mermaid diagram', { error, code: mermaidCode });
            throw error;
        }
    }
    /**
     ...
     */
    extractMermaidDiagrams(markdown) {
        const diagrams = [];
        ...
        logger_1.logger.info(`Extracted ${diagrams.length} Mermaid diagrams`);
        return diagrams;
    }
    /**
     * Replace Mermaid code blocks with image references
     */
    replaceMermaidBlocks(markdown, processedDiagrams, imageBasePath = './images') {
    ...
    async cleanup() {
        try {
            ...
            this.isInitialized = false;
            logger_1.logger.info('Mermaid processor cleaned up');
        }
        catch (error) {
            logger_1.logger.error('Error during Mermaid processor cleanup', { error });
        }
    }
    /**
     * Private method to render diagram
     */
    async renderDiagram(mermaidCode, format, options) {
...
//# sourceMappingURL=mermaid-processor.js.map