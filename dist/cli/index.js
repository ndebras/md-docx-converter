...
class ConverterCLI {
    constructor() {
        this.converter = new index_1.MarkdownDocxConverter();
    }
    /**
     * Initialize CLI
     */
     ...
     */
    async handleConvertCommand(input, options) {
        try {
            this.setupLogging(options.verbose);
            const spinner = (0, ora_1.default)('Converting Markdown to DOCX...').start();
            // Validate input file
            ...
            if (result.success) {
                ...
                console.log(chalk_1.default.green('??? Success:'), `DOCX file created: ${chalk_1.default.cyan(outputPath)}`);
                if (result.metadata) {
                    console.log(chalk_1.default.gray('Statistics:'));
                    console.log(`  Input size: ${utils_1.PerformanceUtils.formatBytes(result.metadata.inputSize)}`);
                    console.log(`  Output size: ${utils_1.PerformanceUtils.formatBytes(result.metadata.outputSize)}`);
                ...
                console.error(chalk_1.default.red('??? Error:'), result.error?.message || 'Unknown error');
                process.exit(1);
            }
        }
        catch (error) {
     ...
     */
    async handleExtractCommand(input, options) {
        try {
            this.setupLogging(options.verbose);
            const spinner = (0, ora_1.default)('Extracting DOCX to Markdown...').start();
            // Validate input file
            const inputPath = path.resolve(input);
            if (!await fs.pathExists(inputPath)) {
                spinner.fail(`Input file not found: ${inputPath}`);
            ...
            if (result.success) {
                ...
                console.log(chalk_1.default.green('??? Success:'), `Markdown file created: ${chalk_1.default.cyan(outputPath)}`);
                if (result.metadata) {
                    console.log(chalk_1.default.gray('Statistics:'));
                    console.log(`  Input size: ${utils_1.PerformanceUtils.formatBytes(result.metadata.inputSize)}`);
                    console.log(`  Output size: ${utils_1.PerformanceUtils.formatBytes(result.metadata.outputSize)}`);
     ...
     */
    async handleBatchCommand(inputDir, options) {
        try {
            this.setupLogging(options.verbose);
            const inputPath = path.resolve(inputDir);
            const outputPath = path.resolve(options.outputDir);
            if (!await fs.pathExists(inputPath)) {
                console.error(chalk_1.default.red('??? Error:'), `Input directory not found: ${inputPath}`);
                process.exit(1);
            }
            // Find input files
            ...
            if (inputFiles.length === 0) {
                console.log(chalk_1.default.yellow('No files found for conversion'));
                return;
            }
            console.log(`Found ${inputFiles.length} files for batch conversion`);
            const spinner = (0, ora_1.default)(`Converting ${inputFiles.length} files...`).start();
            let results;
            ...
            if (failed > 0) {
                ...
                failedFiles.forEach(({ input, result }) => {
                    console.log(`  ${chalk_1.default.red('???')} ${input}: ${result.error?.message || 'Unknown error'}`);
                });
            }
        }
        catch (error) {
    ...
    async handleValidateCommand(input, options) {
        try {
            ...
            const spinner = (0, ora_1.default)('Validating Markdown file...').start();
            const content = await utils_1.FileUtils.readFile(inputPath);
            const validation = await this.converter.validateMarkdown(content);
            if (validation.isValid) {
                spinner.succeed('Markdown validation passed');
                console.log(chalk_1.default.green('??? Valid:'), 'Markdown file is ready for conversion');
            }
            else {
     ...
     */
    async handleStatsCommand(input, options) {
        try {
            this.setupLogging(options.verbose);
            const inputPath = path.resolve(input);
            if (!await fs.pathExists(inputPath)) {
            ...
            console.log(chalk_1.default.cyan('???? File Statistics:'));
     ...
     */
    handleListCommand() {
        console.log(chalk_1.default.cyan('???? Available Templates:'));
    ...
    validateTemplate(template) {
        ...
        console.warn(chalk_1.default.yellow(`Invalid template '${template}', using 'professional-report'`));
        return 'professional-report';
    }
    /**
     * Validate Mermaid theme
     */
    validateMermaidTheme(theme) {
        const validThemes = this.converter.getAvailableMermaidThemes();
        if (validThemes.includes(theme)) {
            return theme;
        }
        console.warn(chalk_1.default.yellow(`Invalid Mermaid theme '${theme}', using 'default'`));
        return 'default';
    }
    /**
